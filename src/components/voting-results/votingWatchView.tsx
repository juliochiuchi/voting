...
type VotingWatchViewProps = {
  title: string
  description: string
  hasManagementAccess: boolean
  accessChipLabel: string
  backTo?: string | null
  backLabel?: string
}
...
export function VotingWatchView({
  title,
  description,
  hasManagementAccess,
  accessChipLabel,
      ...
      void (async () => {
        ...
        try {
          ...
          if (rememberedSelection) {
            const rememberedElection =
              nextElections.find((election) => election.id === rememberedSelection.electionId) ??
              null
            const rememberedElectionStatus = rememberedElection
              ? normalizeStatus(rememberedElection.status)
              : null
  ...
  const resultsRows = React.useMemo(() => {
    const rows = Array.from(activeMemberNameById.entries()).map(([memberId, memberName]) => {
      ...
      const isConfirmAvailable = hasManagementAccess && isPreElected && !confirmationStatus
    ...
    activeMemberNameById,
    electedStatusByMemberId,
    hasManagementAccess,
    preElectedThreshold,
    voteCountsByMemberId,
  ])
  ...
  return (
    <>
      <main className="mx-auto w-full max-w-7xl px-6 py-14">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
            {hasManagementAccess && selectedElection && selectedRound ? (
              <>
                <Button
                  type="button"
                  size="lg"
            ...
            {backTo ? (
              <Button
                ...
                variant="outline"
                className="h-11 rounded-2xl bg-background/15 px-5 hover:bg-background/25"
                onClick={() => navigate({ to: backTo, replace: true })}
              >
                <ArrowLeft />
                {backLabel}
              </Button>
        ...
        {selectableElections.length === 0 ? (
          ...
          <>
            ...
            <VotingResultsTable
              ...
              isLoading={isLoading}
              showConfirmButtons={hasManagementAccess}
                ...
                <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-muted-foreground">
                      {`${confirmedElectedCount} membro(s) com status Eleito nesta eleição.`}
                    </div>
                  </div>
                </div>
      ...
      <ConfirmActionModal
        ...
        description="Tem certeza que deseja concluir? Esta ação altera o status para Concluída."
